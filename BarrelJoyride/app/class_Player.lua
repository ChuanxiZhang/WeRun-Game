...
function Player:new( parentGroup )
	...
	local sheetInfo = require("seiya")
	local myImageSheet = graphics.newImageSheet( "assets/images/seiya.png", sheetInfo:getSheet() )
	local sequences_rollingCoin = {
	    ...
	    {
	        name = "fastRun",
	        frames = { 2,5,8,11,14,17,20 },
	        time = 400,
	        loopCount = 0
	    },
		{
	        name = "dizzy",
	        frames = { 3,6,9,12,15,18,21 },
	        time = 400,
	        loopCount = 0
	    },	    	    
	}
...
function Player:changeLook( parentGroup )
	if parentGroup._hitBonus == 1 then
		self._visual:setSequence("fastRun")
		self._visual:play()
	elseif parentGroup._isOverturn == true then
		self._visual:setSequence("dizzy")
		self._visual:play()		
	else
		self._visual:setSequence("normal")
		self._visual:play()
	end
...
function Player:enterFrame( event )
	...
	if not self._hasReachedStartingPoint then
		self._visual.x = self._visual.x + 5
	end
